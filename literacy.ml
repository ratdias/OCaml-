...
let process_sol (form: formula) (subs: subst): subst option=
  if eval form subs then raise KeepLooking
  else Some subs
...
let tautology_checker_excn1 (f:formula) : subst option =
  ...
  let rec look_for_falses form sublst=
    match sublst with
    |[] -> None
    |sub::subss -> try process_sol form sub with
                  |KeepLooking -> look_for_falses form subss
    in
    (look_for_falses f (all_possible_subst (all_bool_combinations (List.length (freevars f))) (freevars f) []))
  

let process_sol1 (form: formula) (subs: subst): unit=
  if eval form subs then ()
  else raise (FoundCounterExample subs)
...
let tautology_checker_excn2 (f:formula) : subst option =
  ...
  let rec look_for_falses form sublst=
    ...
    |[] -> None
    |sub::subss -> try process_sol form sub with
                  |FoundCounterExample x -> Some x
                  |_ -> look_for_falses form subss
    in
    (look_for_falses f (all_possible_subst (all_bool_combinations (List.length (freevars f))) (freevars f) []))
...
(* (not P or Q) or (not Q or P) 
   - logically the same as t2 *)
let t3 = Or (Or ( Not (Prop "P"), Prop "Q"), Or ( Not (Prop "Q"), Prop "P"))


(* Some formulae that are not tautologies *)
(* P or Q *)
